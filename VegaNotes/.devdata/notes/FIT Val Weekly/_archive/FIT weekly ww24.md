...
@aboli
	...
	#project jnc
		...
		#task T-AX24MR RTL bug fix model has FV failures #status blocked
			...
			#AR T-H66QC7 2 FV fixes #status done
			#AR T-B45J4P 1 Jasper failure - details to be added #status done
...
@Namratha
	#task T-C0WWN0 Cover buckets debug
		#AR T-MJ453V 10 covers unreachable #eta ww19 #status done
	#task T-800631 JNC bucket debug #status in-progress
		#AR T-W7M4KZ MRN counter bucket debug #status done
		#AR T-82SWCP fe::Formal::Assert::cex::idq.fv_idq_mrn.T_FPV_FIT_ID_MRN_AssertStoreAllMrnUpdatesCorrectly::jnc-a0 @njammala #status in-progress
		#AR T-0WK980 fe::Formal::Assert(sec)::cex::idq.IDSilentUBranchM*H::jnc-a0 @njammala
...
@Ragavi
	#project jnc
		#task T-3CF48M ITLB preloader SMT coding and smart preloader #status in-progress
			...
			#AR T-WZW8XS smt coding #status done
		...
		#task T-2953KW Snoop injector SMT coding #eta ww19 #status in-progress
			#AR T-XN2WQY further breakdown of tasks #eta ww18.2 #status done
			#AR T-6XJAS5 Coding done #eta ww18 #status done
...
@Gautham
	#project jnc
		#task T-WCF4H6 updating coverage for fe_idq_tlm_cov.e for SMT #status wip #eta WW25 #priority P0
			#AR T-PZQ4CK updated code to handle threads @Gautham #status done
			#AR T-4SPN38 add missing signals to packet @Gautham #status done
			#AR T-ZVSM2W verify cover groups collected @Gautham #status done
			#AR T-VFRC9F turn in without thread-aware 148h signals @Gautham #status done
			#AR T-P51GT4 investigate 148H signals @Gautham #status done
			#AR T-Q1C4MS update coverage to handle 148h signals @Gautham #status done
			#AR T-5A2J1D debugging why more failing tests @gajith #status done
			#AR T-5YB2ZX verify lsd coverage w MT1 / MT0 comparison @gajith #status done
			#AR T-7FZYNJ debug missing coverage scores for MT0 @gajith #status done
@Yongxi
	#project csk
@Niharika
	#project jnc
...
#task T-P7QS48 Full Code coverage GFC @Gautham #status in-progress #eta 2026-ww19
	...
	#AR T-FJ0M34 make required changes based off doc/email @Gautham #status done
	#AR T-G9W18Y run coverage @Gautham #status done
	#AR T-B28GKB run full regression suite @Gautham #status in-progress
	#AR T-90JRHP Build and send model to Aya @gajith #status done
...
#task T-5WGW2J WW24_JNC_Bucket_Debug @khbyers #status in-progress #eta WW24.5
	#AR T-0JK45W DSB Thread hang due to non-threaded dsbq ctl sync signal @khbyers #status done
	#AR T-XNRPX1 branch skid issue with nuke BPU assertion @khbyers #status done
	#AR T-YTVX7Y DSBQ Full Stall @khbyers
